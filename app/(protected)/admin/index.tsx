...
export default function AdminOverviewScreen() {
  ...
  if (!data) {
    return <AdminLoadingState label="Loading operational overview" />;
  }
    ...
    <>
      ...
      <AdminPanel>
        <InteractiveTimelineChart
          ...
          subtitle="Search and AI request volume on a shared count scale"
      ...
      <YStack>
        <XStack gap={10} flexWrap="wrap" alignItems="stretch">
          <YStack flex={1} minWidth={260}>
            <AdminPanel>
              <YStack gap={12}>
                <AdminSectionHeader title="Provider comparison" />
                <BarChart
                  rows={data.comparison.provider.map(
                    (row: { key: string; requests: number; failureRate: number }) => ({
                      ...
                      status: row.failureRate > 0.08 ? "failure" : "default",
                    }),
                  )}
                />
              </YStack>
          ...
          <YStack flex={1} minWidth={260}>
            <AdminPanel>
              <YStack gap={12}>
                <AdminSectionHeader title="Capability comparison" />
                <BarChart
